...
class AssignmentSubmissionViewSet(BaseModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        # Accept multipart/form-data with optional file under 'file' and other fields
        ...
        try:
            # handle optional file upload or submitted_file_url
            file = request.FILES.get('file')
            submitted_file_url = None
            if file:
                # Basic validation: size and mime (if provided)
                ...
                # reuse default_storage to save the file (works for local or S3)
                try:
                    # Sanitize and namespace the filename to avoid collisions and
                    # problems with client-provided names. Use a UUID prefix.
                    ...
                    # Attempt a local fallback to MEDIA_ROOT if default_storage fails
                    print('default_storage.save failed, attempting MEDIA_ROOT fallback:', e)
                    traceback.print_exc()
                    try:
                        ...
                        local_dir = os.path.join(str(media_root), 'assignment_submissions')
                        os.makedirs(local_dir, exist_ok=True)
                        filename = f"{uuid.uuid4().hex}_{safe_name}"
                        local_path = os.path.join(local_dir, filename)
                        # write file chunks to local path
                        file.seek(0)
                        ...
                        if not media_url.endswith('/'):
                            media_url = media_url + '/'
                        submitted_file_url = media_url + f'assignment_submissions/{filename}'
                    except Exception as e2:
                        print('Fallback save to MEDIA_ROOT also failed:', e2)
                        traceback.print_exc()
                        return Response({'detail': 'Failed to save uploaded file to storage.', 'error': str(e2)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            else:
                # allow clients to provide an existing file URL
            ...
            # handle DB uniqueness or integrity issues explicitly
            print('IntegrityError saving submission:', ie)
            return Response({'detail': 'Database integrity error saving submission.', 'error': str(ie)}, status=status.HTTP_409_CONFLICT)
        except Exception as e:
            # unexpected error: log and return 500 with brief message
            print('Unexpected error in AssignmentSubmission.create:', e)
            return Response({'detail': 'Unexpected server error saving submission.', 'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class TestSubmissionViewSet(BaseModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            print('IntegrityError saving test submission:', ie)
            return Response({'detail': 'Database integrity error saving test submission.', 'error': str(ie)}, status=status.HTTP_409_CONFLICT)
        except Exception as e:
            print('Unexpected error in TestSubmission.create:', e)
            return Response({'detail': 'Unexpected server error saving test submission.', 'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class LiveClassViewSet(BaseModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        """Wrap create in try/except to surface unexpected server errors during development."""
        try:
            serializer = self.get_serializer(data=request.data)
            serializer.is_valid(raise_exception=True)
            try:
                # normal flow: let perform_create handle ownership fields
                self.perform_create(serializer)
                headers = self.get_success_headers(serializer.data)
                return Response(serializer.data, status=status.HTTP_201_CREATED, headers=headers)
            except (IntegrityError, DatabaseError) as db_err:
                msg = str(db_err)
                # Detect known schema mismatch where DB still has `participants` as NOT NULL
                if 'participants' in msg or "Field 'participants'" in msg or 'doesn\'t have a default value' in msg:
                    # Attempt a compatibility fallback: perform a direct INSERT including participants=0
                    try:
                        model = getattr(serializer.Meta, 'model', None)
                        table = model._meta.db_table
                        user = getattr(request, 'user', None)

                        # Build columns/values from validated_data
                        vd = dict(serializer.validated_data)
                        cols = []
                        params = []
                        for k, v in vd.items():
                            # map ForeignKey fields to <field>_id column
                            try:
                                f = model._meta.get_field(k)
                                if hasattr(f, 'remote_field') and f.remote_field is not None:
                                    col = f.column  # e.g., course_id
                                    val = getattr(v, 'pk', v)
                                else:
                                    col = f.column
                                    val = v
                            except Exception:
                                # conservative fallback: use raw key
                                col = k
                                val = getattr(v, 'pk', v) if hasattr(v, 'pk') else v
                        ...
                        # ensure instructor is set when applicable
                        if 'instructor_id' in [c for c in cols]:
                            pass
                        else:
                            if user and getattr(user, 'role', None) in ('teacher', 'admin'):
                                cols.append('instructor_id')
                                params.append(user.pk)

                        # ensure participants column present with safe default 0
                        if 'participants' not in cols and 'participants' not in [c for c in cols]:
                            cols.append('participants')
                            params.append(0)

                        # include created_at / updated_at timestamps if table expects them
                        ...
                        print('LiveClass fallback INSERT SQL:', sql, 'params:', params)
                        with connection.cursor() as cur:
                            cur.execute(sql, params)
                            last_id = cur.lastrowid
                        # return the created instance
                        instance = model.objects.get(pk=last_id)
                        ...
                        print('Fallback INSERT failed for LiveClass:', fallback_exc)
                        print(tb)
                        return Response({'detail': 'Unexpected server error creating LiveClass (fallback failed).', 'error': str(fallback_exc), 'trace': tb}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
                # not a participants-related DB error: re-raise to be handled below
                raise
        except Exception as e:
            import traceback
            tb = traceback.format_exc()
            # Log to server stdout for developer diagnosis
            print('Unexpected error in LiveClassViewSet.create:', e)
            print(tb)
            # Return a helpful error payload (dev only)
            return Response({'detail': 'Unexpected server error creating LiveClass.', 'error': str(e), 'trace': tb}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class AnnouncementViewSet(BaseModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            serializer = self.get_serializer(data=request.data)
            serializer.is_valid(raise_exception=True)
            # perform_create will set created_by if available
            try:
                self.perform_create(serializer)
            except Exception:
                # If perform_create fails, still attempt serializer.save()
                serializer.save()
            headers = self.get_success_headers(serializer.data)

            # After creating the announcement, attempt to send notifications for selected channels.
            try:
                # Reload instance to get full model (including JSON fields)
                            ...
                            if recipient_list:
                                ...
                                for i in range(0, len(recipient_list), batch_size):
                                    ...
                                    try:
                                        ...
                                        logger.debug(tb)
                                        # Fallback: attempt to print to console backend so devs can see output
                                        try:
                                            conn = get_connection('django.core.mail.backends.console.EmailBackend')
                                            emails = []
                                            for rcpt in batch:
                                                emails.append(EmailMessage(subject=subject, body=message, from_email=from_email, to=[rcpt], connection=conn))
                                            conn.send_messages(emails)
                                            logger.info('Fallback: printed announcement email batch (%d-%d) to console backend', i, i+len(batch))
                                        except Exception as e_fallback:
                                            tb2 = traceback.format_exc()
                                            logger.exception('Fallback console backend also failed for batch (%d-%d): %s', i, i+len(batch), e_fallback)
                                            logger.debug(tb2)
                        except Exception as e:
                            tb_outer = traceback.format_exc()
                            logging.getLogger(__name__).exception('Failed to prepare/send announcement emails: %s', e)
                            logging.getLogger(__name__).debug(tb_outer)
                            ...
                            for phone in phone_qs.values_list('phone', flat=True):
                                try:
                                    ...
                                    if r.status_code >= 400:
                                        print('Twilio SMS send failed for', phone, 'status', r.status_code, r.text)
                                except Exception as e:
                                    print('Error sending SMS to', phone, e)
                        else:
                            print('SMS channel requested but Twilio not configured in settings; skipping SMS sends.')

            except Exception as notify_exc:
                # Do not fail announcement creation if notifications fail; just log
                print('Error while sending announcement notifications:', notify_exc)
            ...
            print('Unexpected error in AnnouncementViewSet.create:', e)
            print(tb)
            # Return a helpful error payload (dev-friendly)
            return Response({'detail': 'Unexpected server error creating Announcement.', 'error': str(e), 'trace': tb}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class UploadViewSet(BaseModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Return an informative error to the client for easier debugging
            import traceback
            tb = traceback.format_exc()
            return Response({'detail': 'Failed to save uploaded file', 'error': str(e), 'trace': tb}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
